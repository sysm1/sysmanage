...
public class ImportController extends BaseController {
	...
	**/
	...
	public String exportCloth(Model model, HttpServletRequest request,
			HttpServletResponse response, String pageNow, String pagesize) {
		...
		model.addAttribute("pageView",
				orderSummaryService.queryReportByClothPage(pageView, map));

		return Common.BACKGROUND_PATH + "/report/cloth";
	}

	@RequestMapping("exportFactory")
	public String exportFactory(Model model, HttpServletRequest request,
			HttpServletResponse response, String pageNow, String pagesize) {
		Map<String, Object> map = new HashMap<String, Object>();
		String beginTime = request.getParameter("beginTime");
		String endTime = request.getParameter("endTime");
		...
		model.addAttribute("pageView",
				orderSummaryService.queryReportByFactoryPage(pageView, map));

		return Common.BACKGROUND_PATH + "/report/factory";
	}

	@RequestMapping("exportClothExcel")
	public void exportClothExcel(HttpServletRequest request,
		...
		map.put("endTime", endTime);
		map.put("clothId", clothId);
		List<OrderReportClothVO> acs = orderSummaryService
				.queryReportByCloth(map);
		POIUtils.exportToExcel(response, "下单报表(布种)", acs,
				OrderReportClothVO.class, "下单报表(布种)", acs.size());
	}

	@RequestMapping("exportFactoryExcel")
	public void exportFactoryExcel(HttpServletRequest request,
			HttpServletResponse response) {
		Map<String, Object> map = new HashMap<String, Object>();
		String beginTime = request.getParameter("beginTime");
		String endTime = request.getParameter("endTime");
		...
		map.put("endTime", endTime);
		map.put("factoryId", factoryId);
		List<OrderReportFactoryVO> acs = orderSummaryService
				.queryReportByFactory(map);
		POIUtils.exportToExcel(response, "下单报表(工厂)", acs,
				OrderReportFactoryVO.class, "下单报表(工厂)", acs.size());
	}
	...
	public void exportSampleExcel(HttpServletRequest request,
			HttpServletResponse response) {
		Map<String, Object> map = new HashMap<String, Object>();
		String beginTime = request.getParameter("beginTime");
		String endTime = request.getParameter("endTime");
			...
			for (SampleAdditional si : list) {
				if (vo.getId().equals(si.getSampleId())) {
					if (null == facotoryCodeMap.get(si.getFactoryCode())) {
						facotoryCodeMap.put(si.getFactoryCode(),
								new ArrayList<String>());
					}
					facotoryCodeMap.get(si.getFactoryCode()).add(
							si.getFactoryColor());
				}
			}
		...
		POIUtils.exportToExcel(response, "开版进度查询", acs, SampleInputVO.class,
				"开版进度查询", acs.size());
	}

	@RequestMapping(value = "flowerFileImport", method = RequestMethod.POST)
	public @ResponseBody Map<String, Object> fileUpload(
			@RequestParam("flowerFile") CommonsMultipartFile flowerFile,
			HttpSession session, HttpServletRequest request,
			HttpServletResponse response) {

		Map<String, Object> map = new HashMap<String, Object>();
		String filename = flowerFile.getOriginalFilename();
		try {
			
			List<List<String>> list = POIUtils.getExcelData(flowerFile,filename, 0);
