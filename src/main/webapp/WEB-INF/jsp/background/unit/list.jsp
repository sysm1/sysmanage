...
<script type="text/javascript">
	...
	$(function() {
		grid = window.lanyuan.ui.lyGrid({
					...
					jsonUrl : '${pageContext.request.contextPath}/background/unit/query.html',
					checkbox : true
				});
		...
		$("#exportExcel").click("click", function() {//绑定查询按扭
			...
			f.attr('action','${pageContext.request.contextPath}/background/unit/exportExcel.html');
			f.submit();
		});
		...
		$("#add").click("click", function() {//绑定查询按扭
			dialog = parent.$.ligerDialog.open({
				width : 300,
				height : 310,
				url : rootPath + '/background/unit/addUI.html',
				title : "增加布种单位信息",
		...
		$("#editView").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 300,
				height : 310,
				url : rootPath + '/background/unit/editUI.html?id='+cbox,
				title : "修改布种单位信息",
				isHidden : false
			});
		});
			...
			parent.$.ligerDialog.confirm('删除后不能恢复，确定删除吗？', function(confirm) {
				if (confirm) {
					$.ajax({
					    ...
					    url: rootPath + '/background/unit/deleteById.html', //要访问的后台地址
