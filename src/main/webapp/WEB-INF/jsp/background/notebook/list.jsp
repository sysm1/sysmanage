...
<script type="text/javascript">
	...
	$(function() {
		grid = window.lanyuan.ui.lyGrid({
					...
					jsonUrl : '${pageContext.request.contextPath}/background/notebook/query.html',
					checkbox : true
				});
		...
		$("#exportExcel").click("click", function() {//绑定查询按扭
			...
			f.attr('action','${pageContext.request.contextPath}/background/notebook/exportExcel.html');
			f.submit();
		});
		...
		$("#add").click("click", function() {//绑定查询按扭
			dialog = parent.$.ligerDialog.open({
				width : 300,
				height : 310,
				url : rootPath + '/background/notebook/addUI.html',
				title : "增加日志信息",
		...
		$("#editView").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 300,
				height : 310,
				url : rootPath + '/background/notebook/editUI.html?id='+cbox,
				title : "修改布种信息",
				isHidden : false
			});
		});
			...
			parent.$.ligerDialog.confirm('删除后不能恢复，确定删除吗？', function(confirm) {
				if (confirm) {
					$.ajax({
					    ...
					    url: rootPath + '/background/notebook/deleteById.html', //要访问的后台地址
	...
	<div class="divBody">
		<div class="search">
			<form name="fenye" id="fenye">
				...
				内容：<input type="text" name="content" value="${param.content}" style="height: 20px" />
