...
<div class="divdialog">
	...
	<form name="form" id="form" action="${ctx}/background/diaobo/add.html" method="post">
		...
		<table style="width: 400px; height: 100%;" border="1" class="dataintable">
				...
				<td style="text-align: left;">
					<div class="lanyuan_input">
						<input type="text" id="riqi" name="riqi" 
							value="<fmt:formatDate value="${bean.riqi}" pattern="YYYY-MM-dd"/>" 
			...
			<tr>
				...
				<td style="text-align: left;">
					<select id="dcdanwei" name="dcdanwei">
						<option value="">请选择单位</option>
						<c:forEach items = "${ factorys }" var="factory">
							<option value="${factory.id }" <c:if test="${bean.dcdanwei eq factory.id }">selected="selected"</c:if>  >${factory.name}</option>
						</c:forEach>
					</select>
				</td>
			</tr>
					...
					<!--div class="lanyuan_input">
						<select id="clothId" name="clothId">
							...
							<c:forEach items="${ cloths }" var = "cloth">
								<option runit="${cloth.unit}" value="${cloth.id }" <c:if test="${bean.clothId eq cloth.id }">selected="selected"</c:if>>${cloth.clothName}</option>
							</c:forEach>
						</select>
			...
			<tr id="textAllowance"  <c:if test="${clothInfo.unit eq 1}">style="display: none"</c:if>  >
				<td style="text-align: right;">条数：</td>
				<td style="text-align: left;" nowrap="nowrap" style="width: 290px;">
					<input id='allowance' name="allowance" type="text" class="isNum" value="${bean.num }" >
				</td>
			</tr><tr>
				<td style="text-align: right;">公斤数：</td>
				<td style="text-align: left;" nowrap="nowrap" style="width: 290px;">
					<input id='numKg' name="numKg" type="text" class="isNum" value="${bean.numKg }" >
				</td>
			</tr><tr>
				<td style="text-align: right;">调入单位：</td>
				<td style="text-align: left;" nowrap="nowrap" style="width: 290px;">
					<select id="drdanwei" name="drdanwei">
						<option value="">请选择单位</option>
						<c:forEach items = "${ factorys }" var="factory">
							<option value="${factory.id }" <c:if test="${bean.drdanwei eq factory.id }">selected="selected"</c:if>>${factory.name}</option>
						</c:forEach>
					</select>
				</td>
			</tr><tr>
				...
				<td style="text-align: left;">
					<textarea rows="4" style="width: 90%" id='mark' name="mark">${bean.mark }</textarea>
				</td>
			</tr><tr>
				<td colspan="2">
