...
jQuery.validator.addMethod("isNum", function(value, element) {
	...
	$(function() {
		...
		$("#seach").click("click", function() {//绑定查询按扭
			...
			$("#delay").attr('value','');
			var f = $('#fenye');
			f.attr('action','${pageContext.request.contextPath}/background/diaobo/list.html');
			f.submit();
		});
		...
		$("#add").click("click", function() {//绑定查询按扭
			dialog = parent.$.ligerDialog.open({
				width : 420,
				height : 450,
				url : rootPath + '/background/diaobo/addUI.html',
				title : "增加坯布采购单",
		...
		$("#editView").click("click", function() {//绑定查询按扭
			...
			dialog = parent.$.ligerDialog.open({
				width : 420,
				height : 450,
				url : rootPath + '/background/diaobo/editUI.html?id='+cbox,
				title : "坯布采购单修改",
				isHidden : false
			});
		});
			...
			parent.$.ligerDialog.confirm('删除后不能恢复，确定删除吗？', function(confirm) {
				if (confirm) {
					$.ajax({
					    ...
					    url: rootPath + '/background/diaobo/deleteById.html', //要访问的后台地址
				...
				<c:forEach var="item" items="${pageView.records }" varStatus="status">
					<tr  id="${status.index }" >
						<td style="width:50px;text-align: center;">
					 		...
					 		<fmt:formatDate value="${item.riqi }" pattern="yyyy-MM-dd"/>
					 	</td><td style="width: 110px;">
					 		${item.dcdanweiName }
					 	</td><td>
					 		${item.clothName }
					 	</td><td>
					 		<c:if test="${item.num == null }">-</c:if>
					 		<c:if test="${item.num != null }">${item.num }</c:if>
					 	</td><td>
					 		${item.numKg }
					 	</td><td>
					 		${item.drdanweiName }
					 	</td><td style="width: 250px;" title="${item.mark }">
